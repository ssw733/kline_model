...
from .config import DbConfig, SourceConfig
...
def _verdict(mean_value: float, positive_rate: float) -> str:
    if mean_value >= 0.02 and positive_rate >= 0.6:
        return "bullish"
    if mean_value <= -0.02 and positive_rate <= 0.4:
        return "bearish"
    return "neutral"
...
def _shape_features(close_prices: np.ndarray) -> dict[str, np.ndarray | float]:
    ...
    log_returns = np.diff(np.log(np.clip(close_prices, 1e-12, None)), prepend=np.log(max(close_prices[0], 1e-12)))
    log_returns[0] = 0.0
...
def _window_similarity(
    ...
    shape_cosine = float(
        ...
        / ((np.linalg.norm(query_feat["zrel"]) + 1e-8) * (np.linalg.norm(match_feat["zrel"]) + 1e-8))
    )
    path_mae = float(np.mean(np.abs(query_feat["rel"] - match_feat["rel"])))
    path_score = 1.0 / (1.0 + path_mae * 25.0)
    final_score = 0.55 * embedding_score + 0.30 * shape_cosine + 0.15 * path_score
    return {
...
def _build_plot_series(
    ...
    open_rel = entity_series["open"][start_idx:plot_end_idx] / max(float(slice_prices[0]), 1e-12) - 1.0
    high_rel = entity_series["high"][start_idx:plot_end_idx] / max(float(slice_prices[0]), 1e-12) - 1.0
    low_rel = entity_series["low"][start_idx:plot_end_idx] / max(float(slice_prices[0]), 1e-12) - 1.0
...
def _save_plot(
    ...
    colors = ["#1d4ed8", "#ea580c", "#16a34a", "#dc2626", "#7c3aed", "#0891b2"]
    ...
    for panel_idx, panel in enumerate(panels):
        ...
        for series in panel["series"]:
            ...
            if series.get("kind") == "candles":
                ...
                candle_body = max(min(plot_width / max(window_size * 1.8, 1), 10), 3)
                for x_val, open_val, high_val, low_val, close_val in zip(
                    ...
                    body_color = "#16a34a" if close_val >= open_val else "#dc2626"
                    svg_parts.append(
    ...
    output_path = os.path.join(plot_dir, f"pattern_search_{query_item['entity_id']}.svg")
...
def search(
    ...
    metadata, embeddings = load_artifacts(artifact_dir)
    windows = metadata["windows"]
    train_config = metadata["train_config"]
    source_cfg_payload = metadata.get("source_config", {"source_kind": "binance"})
    need_series_lookup = bool(horizons or plot_dir or True)
    series_lookup = _load_series_lookup(source_cfg_payload) if need_series_lookup else {}
    prices_by_entity = {
        entity_id: values["price"] for entity_id, values in series_lookup.items()
    } if horizons else {}
    ...
    if entity_id:
        ...
        if not candidate_indexes:
            raise ValueError(f"Entity '{entity_id}' was not found in saved windows.")
    ...
    print(
        f"query_idx={query_idx} entity={query_item['entity_id']} "
    ...
    if only_past:
        print(
            f"search_mode=historical_only gap_candles={gap_candles} "
            f"calendar_gap_candles={calendar_gap_candles} "
            f"rule=match_end_before_query_start_minus_calendar_gap"
        )
    print(
        f"shape_filters=min_range_ratio={min_range_ratio} max_range_ratio={max_range_ratio} "
        f"min_vol_ratio={min_vol_ratio} max_vol_ratio={max_vol_ratio} min_shape_score={min_shape_score}"
    )
    if horizons:
        query_outcomes = []
        ...
        if query_outcomes:
            print("query_outcomes", " ".join(query_outcomes))
    for final_score, idx, item, similarity in candidates:
        line = (
            f"rank={printed + 1} idx={idx} score={final_score:.4f} "
        ...
        if horizons:
            ...
            if outcome_parts:
                line = f"{line} outcomes={' '.join(outcome_parts)}"
            else:
                line = f"{line} outcomes=n/a"
        print(line)
    ...
    if horizons:
        ...
        for horizon in horizons:
            ...
            print(
                f"  +{horizon}: mean={mean_value:.2%} median={median_value:.2%} "
